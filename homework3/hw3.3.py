...
price = {"A": 1, "E": 1, "I": 1, "O": 1, "U": 1, "L": 1, "N": 1, "S": 1, "T": 1,
...
print(sum([price.get(word[i]) for i in range(len(word))]))
...
for i in ang_dict.items(): 
    # 
    for j in word:   #перебираем каждую букву слова
      if j.upper() in i[0]:  #если буква в верхнем регистре присутствует в ключе
        i[1] #то мы ее забираем 
        # и из кортежа, который создает команда items,
        # забираем второе (1ое по индексу) значение
